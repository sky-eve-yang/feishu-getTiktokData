...
const toCalcInterCount = ref(['viewCount', 'likeCount', 'shareCount', 'collectionCount', 'commentCount'])  // 实际用于计算“总交互量”的字段
...
const writeData = async () => {
  ...
  for (let recordId of RecordList) {
    ...
    // 错误处理：链接格式错误
    if (!noteLink.includes('tiktok.com')) {
...
const getDataByCheckedFields = async (noteLink) => {
  ...
  const path = 'tiktok'
  let basicInfo = await getTiktokDataByLink(path, noteLink)
